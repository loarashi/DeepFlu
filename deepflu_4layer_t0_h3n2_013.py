...
all_df = pd.read_csv("data/t0_data/H3N2_rmat0_013_t0.txt",sep='\t',encoding='utf-8')#讀取H3N2_rmat0_013_t0.txt檔案
...
test_Features=all_Features[:1]#提取測試資料的特徵(H3N2的13號受試者只有一個時間點)
test_Label=all_Label[:1]#提取測試資料的標籤(H3N2的13號受試者只有一個時間點)
...
predict=all_probability[:1]#提取測試資料的預測標籤(H3N2的13號受試者只有一個時間點)
predict_score=all_probability_score[:1]#提取測試資料的預測機率(H3N2的13號受試者只有一個時間點)
predict=predict.tolist()
predict_score=predict_score.tolist()

f1 = open("./H1N1_t0_001.txt", 'a', encoding = 'UTF-8')#可自行取名儲存檔案
f1.write(str(int(test_Label[0]))+"\t"+str(predict[0])+"\t"+str(predict_score[0])+"\n")#列印測試資料標籤/測試資料預測標籤(一個時間點)/測試資料預測機率(一個時間點)
f1.close
